...
def test_encryption_certificates_uses_public_client(runner) -> None:
    ...
    assert service.called("get_certificates")["usage"] == ["invoice"]
...
def test_peppol_providers_query_and_all(runner) -> None:
    ...
    result = runner.invoke(app, cli_args("peppol", "providers", "--page-size", "10"), obj=runtime)
...
def test_limits_get_set_reset_and_production(runner, tmp_path) -> None:
    ...
    runtime = fake_runtime(auth=auth_obj, model_reader=lambda path, model_type: {"payload": str(path)})

    assert payload(runner.invoke(app, cli_args("limits", "get", "api"), obj=runtime)) == {"kind": "api"}
    assert payload(runner.invoke(app, cli_args("limits", "get", "context"), obj=runtime)) == {"kind": "context"}
    assert payload(runner.invoke(app, cli_args("limits", "get", "subject"), obj=runtime)) == {"kind": "subject"}

    invalid = runner.invoke(app, cli_args("limits", "get", "missing"), obj=runtime)
    assert invalid.exit_code == 1
    assert "kind must be one of" in invalid.output

    payload_file = tmp_path / "limits.json"
    payload_file.write_text("{}", encoding="utf-8")
    assert payload(runner.invoke(app, cli_args("limits", "set", "api", "--payload", str(payload_file)), obj=runtime)) == {
        "kind": "api",
        "updated": "true",
    }
    assert service.called("set_api_rate_limits")["limits"] == {"payload": str(payload_file)}

    assert payload(runner.invoke(app, cli_args("limits", "reset", "session"), obj=runtime)) == {
        "kind": "session",
        "reset": "true",
    }
    assert payload(runner.invoke(app, cli_args("limits", "production-rate-limits"), obj=runtime)) == {
...
def test_tokens_commands(runner) -> None:
    service = FakeService(
        ...
        list_all=lambda **kwargs: [type("Page", (), {"tokens": [{"reference_number": "r1"}]})()],
        status={"status": "active"},
        revoke=None,
    )
    runtime = fake_runtime(auth=type("Auth", (), {"tokens": service})())

    missing = runner.invoke(app, cli_args("tokens", "generate", "--description", "demo"), obj=runtime)
    assert missing.exit_code == 1
    ...
    assert payload(
        runner.invoke(
            app,
            cli_args("tokens", "generate", "--description", "demo", "--permission", "invoice_read"),
            obj=runtime,
        )
    ) == {"token": "secret"}
    assert service.called("generate")["permissions"] == ["invoice_read"]

    assert payload(runner.invoke(app, cli_args("tokens", "list", "--status", "active"), obj=runtime)) == {
        "tokens": [{"reference_number": "r1"}]
    }
    assert payload(runner.invoke(app, cli_args("tokens", "list", "--all"), obj=runtime)) == [{"reference_number": "r1"}]
    assert payload(runner.invoke(app, cli_args("tokens", "status", "--reference", "r1"), obj=runtime)) == {
        "status": "active"
    }
    assert payload(runner.invoke(app, cli_args("tokens", "revoke", "--reference", "r1"), obj=runtime)) == {
        "reference_number": "r1",
        "revoked": "true",
    }
...
def test_sessions_commands(runner) -> None:
    session_service = FakeService(
        ...
        all=lambda **kwargs: [type("Page", (), {"items": [{"reference_number": "auth-ref"}]})()],
        close=None,
        terminate_current=None,
    )
    invoice_service = FakeService(
        query={"sessions": [{"reference_number": "invoice-ref"}]},
        all=lambda **kwargs: [type("Page", (), {"sessions": [{"reference_number": "invoice-ref"}]})()],
    )
    auth_obj = type(
        "Auth",
        (),
    ...
    assert payload(runner.invoke(app, cli_args("sessions", "auth-list"), obj=runtime)) == {
        "items": [{"reference_number": "auth-ref"}]
    }
    assert payload(runner.invoke(app, cli_args("sessions", "auth-list", "--all"), obj=runtime)) == [
        {"reference_number": "auth-ref"}
    ]
    assert payload(runner.invoke(app, cli_args("sessions", "auth-close", "--reference", "auth-ref"), obj=runtime)) == {
        "reference_number": "auth-ref",
        "closed": "true",
    }
    assert payload(runner.invoke(app, cli_args("sessions", "auth-terminate-current"), obj=runtime)) == {
        "terminated_current": "true"
    }
    assert payload(runner.invoke(app, cli_args("sessions", "invoice-list", "--type", "online"), obj=runtime)) == {
        "sessions": [{"reference_number": "invoice-ref"}]
    }
    assert payload(runner.invoke(app, cli_args("sessions", "invoice-list", "--type", "batch", "--all"), obj=runtime)) == [
